...
class SingleViewNetwork_DFKI(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.conv_1_1 = nn.Conv2d(
            in_channels=32, out_channels=32, kernel_size=7, padding=3)
        self.bn_1 = nn.BatchNorm2d(32)
        self.conv_2_0 = nn.Conv2d(
        ...
        self.conv_14_0 = nn.Conv2d(
            in_channels=128 + 64, out_channels=64, kernel_size=3, padding=1)
        self.conv_14_1 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.bn_14 = nn.BatchNorm2d(64)

        self.conv_15_0 = nn.Conv2d(
            in_channels=32 + 64, out_channels=64, kernel_size=3, padding=1)
        self.conv_15_1 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.bn_15 = nn.BatchNorm2d(64)

        self.conv_16_0 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.conv_16_1 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.bn_16 = nn.BatchNorm2d(64)
        # self.batch16 = nn.BatchNorm2d(64)
        self.output = nn.Conv2d(in_channels=64, out_channels=configs['num_planes'] +
                                configs['occlusion_levels'] * configs['num_planes'], kernel_size=3, padding=1)
    ...
    def forward(self, rgb):
        ...
        conv2 = F.relu(
            (self.conv_2_1(F.relu(self.conv_2_0(F.max_pool2d(conv1, kernel_size=2))))))

        conv3 = F.relu(
            (self.conv_3_1(F.relu(self.conv_3_0(F.max_pool2d(conv2, kernel_size=2))))))

        conv4 = F.relu(
            (self.conv_4_1(F.relu(self.conv_4_0(F.max_pool2d(conv3, kernel_size=2))))))

        conv5 = F.relu(
            (self.conv_5_1(F.relu(self.conv_5_0(F.max_pool2d(conv4, kernel_size=2))))))

        conv6 = F.relu(
            (self.conv_6_1(F.relu(self.conv_6_0(F.max_pool2d(conv5, kernel_size=2))))))

        conv7 = F.relu(
            (self.conv_7_1(F.relu(self.conv_7_0(F.max_pool2d(conv6, kernel_size=2))))))

        conv8 = F.relu(
            (self.conv_8_1(F.relu(self.conv_8_0(F.max_pool2d(conv7, kernel_size=2))))))

        # Decoding
        up_8 = F.interpolate(conv8, scale_factor=2, mode='nearest')
        conv9 = F.relu(
            (self.conv_9_1(F.relu(self.conv_9_0(torch.cat([up_8, conv7], dim=1))))))

        up_9 = F.interpolate(conv9, scale_factor=2, mode='nearest')
        conv10 = F.relu(
            (self.conv_10_1(F.relu(self.conv_10_0(torch.cat([up_9, conv6], dim=1))))))

        up_10 = F.interpolate(conv10, scale_factor=2, mode='nearest')
        conv11 = F.relu(
            (self.conv_11_1(F.relu(self.conv_11_0(torch.cat([up_10, conv5], dim=1))))))

        up_11 = F.interpolate(conv11, scale_factor=2, mode='nearest')
        conv12 = F.relu(
            (self.conv_12_1(F.relu(self.conv_12_0(torch.cat([up_11, conv4], dim=1))))))

        up_12 = F.interpolate(conv12, scale_factor=2, mode='nearest')
        conv13 = F.relu(
            (self.conv_13_1(F.relu(self.conv_13_0(torch.cat([up_12, conv3], dim=1))))))

        up_13 = F.interpolate(conv13, scale_factor=2, mode='nearest')
        conv14 = F.relu(
            (self.conv_14_1(F.relu(self.conv_14_0(torch.cat([up_13, conv2], dim=1))))))

        up_14 = F.interpolate(conv14, scale_factor=2, mode='nearest')
        conv15 = F.relu(
            (self.conv_15_1(F.relu(self.conv_15_0(torch.cat([up_14, conv1], dim=1))))))
