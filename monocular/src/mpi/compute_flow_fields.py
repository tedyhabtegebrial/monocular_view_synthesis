...
class ComputeFlowFields(nn.Module):
	...
	def get_grid(self, h_matrix, h, w):
		...
		xy_locations = self.apply_h_mats.get_homogeneous_xy_locations(b_size * num_d, h, w).to(device_)
	...
	def forward(self, h_mats, warped_alphas):
		...
		grid = self.alpha_composite(mul_plane_grid, warped_alphas).permute(0, 2, 3, 1)
		return grid
